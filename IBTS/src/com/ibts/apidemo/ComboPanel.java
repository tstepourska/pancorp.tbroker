...
import com.ibts.apidemo.OrdersPanel.OrderRow;
import com.ibts.apidemo.OrdersPanel.OrdersModel;
import com.ibts.apidemo.TopModel.TopRow;
...
import com.ibts.controller.DeltaNeutralContract;
import com.ibts.controller.NewComboLeg;
...
import com.ibts.controller.ApiController.IEfpHandler;
...
public class ComboPanel extends JPanel implements INewTab {
	...
	@Override public void activated() {
		com.ibts.apidemo.ApiDemo.INSTANCE.controller().reqLiveOrders( m_ordersModel);
	}
	...
	static class SpreadsPanel extends JPanel {
		...
		protected void onAddLeg() {
			...
			com.ibts.apidemo.ApiDemo.INSTANCE.controller().reqContractDetails( m_contract, new IContractDetailsHandler() {
		...
		class DnPanel extends VerticalPanel {
			...
			protected void onAdd() {
				...
				com.ibts.apidemo.ApiDemo.INSTANCE.controller().reqContractDetails(dn, new IContractDetailsHandler() {
					@Override public void contractDetails(ArrayList<NewContractDetails> list) {
						if (list.size() == 1) {
							...
							com.ibts.apidemo.ApiDemo.INSTANCE.show( "DN description does not define a uniqe contract");
							m_dnContract = null;
	...
	static class EfpPanel extends JPanel {
		...
		protected void onCreateEfp() {
			...
			com.ibts.apidemo.ApiDemo.INSTANCE.controller().reqContractDetails( fut, new IContractDetailsHandler() {
				@Override public void contractDetails(ArrayList<NewContractDetails> list) {
					...
					if (list.size() == 2) {
						...
						com.ibts.apidemo.ApiDemo.INSTANCE.show( "Request does not define a valid unique futures contract");
					}
					else {
			...
			com.ibts.apidemo.ApiDemo.INSTANCE.controller().reqContractDetails( stk, new IContractDetailsHandler() {
				@Override public void contractDetails(ArrayList<NewContractDetails> list) {
		...
		static class EfpModel extends AbstractTableModel {
			...
			void addRow(NewContract contract) {
				...
				m_rows.add( row);
				com.ibts.apidemo.ApiDemo.INSTANCE.controller().reqEfpMktData( contract, "", false, row);
