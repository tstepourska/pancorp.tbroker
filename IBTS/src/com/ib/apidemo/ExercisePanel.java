...
public class ExercisePanel extends HorzPanel implements INewTab, IAccountHandler {
	...
	protected void onAcctChanged() {
		...
		if (i != -1) {
			...
			if (!selAcct.equals( m_selAcct) ) {
				...
				com.ib.apidemo.ApiDemo.INSTANCE.controller().reqAccountUpdates(true, m_selAcct, this);
			}
		}
	}
	...
	class ExPanel extends VerticalPanel {
		...
		protected void onExercise() {
			...
			if (i != -1 && account != null) {
				...
				com.ib.apidemo.ApiDemo.INSTANCE.controller().exerciseOption(account, position.contract(), m_combo.getSelectedItem(), m_qty.getInt(), m_override.isSelected() );
			}
		}
	}
	...
	@Override public void activated() {
		for (String account : com.ib.apidemo.ApiDemo.INSTANCE.accountList() ) {
