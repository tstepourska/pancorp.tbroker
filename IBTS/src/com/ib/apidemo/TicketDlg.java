...
public class TicketDlg extends JDialog {
	...
	public TicketDlg(NewContract contract, NewOrder order) {
		super( com.ib.apidemo.ApiDemo.INSTANCE.frame());
	...
	protected void onTransmitOrder() {
		...
		com.ib.apidemo.ApiDemo.INSTANCE.controller().placeOrModifyOrder( m_contract, m_order, new IOrderHandler() {
			@Override public void orderState(NewOrderState orderState) {
				com.ib.apidemo.ApiDemo.INSTANCE.controller().removeOrderHandler( this);
	...
	class OrderPanel extends VerticalPanel {
		final TCombo<String> m_account = new TCombo<String>( com.ib.apidemo.ApiDemo.INSTANCE.accountList().toArray(new String[0]) );
		...
		OrderPanel() {
			...
			m_account.setSelectedItem( m_order.account() != null ? m_order.account() : com.ib.apidemo.ApiDemo.INSTANCE.accountList().get( 0) ); 
