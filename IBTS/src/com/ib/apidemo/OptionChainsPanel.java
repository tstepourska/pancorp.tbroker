...
public class OptionChainsPanel extends JPanel {
	...
	protected void onAdd() {
		...
		com.ib.apidemo.ApiDemo.INSTANCE.controller().reqContractDetails( m_underContract, new IContractDetailsHandler() {
	...
	protected void onRecUnderDetails(ArrayList<NewContractDetails> list) {
		if (list.size() != 1) {
			com.ib.apidemo.ApiDemo.INSTANCE.show( "Error: " + list.size() + " underlying contracts returned");
			return;
		}
		...
		com.ib.apidemo.ApiDemo.INSTANCE.controller().reqContractDetails( optContract, symbolPanel);
	}
	...
	private class ChainPanel extends NewTabPanel implements IContractDetailsHandler, ActionListener  {
		...
		ChainPanel() {
			...
			com.ib.apidemo.ApiDemo.INSTANCE.controller().reqTopMktData( m_underContract, "", false, m_stockListener);
		}
        ...
        @Override public void closed() {
            com.ib.apidemo.ApiDemo.INSTANCE.controller().cancelTopMktData( m_stockListener);
		...
		private class ChainModel extends AbstractTableModel {
            ...
            public void desubscribe() {
                for (ChainRow row : m_list) {
                    com.ib.apidemo.ApiDemo.INSTANCE.controller().cancelOptionMktData( row);
                }
            }
			...
			public void addRow(NewContract contract, boolean snapshot) {
				...
				com.ib.apidemo.ApiDemo.INSTANCE.controller().reqOptionMktData(contract, "", snapshot, row);
